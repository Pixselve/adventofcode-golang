...
func NewInstruction(instruction int) Instruction {
	return Instruction{
		opcode:     uint(instruction % 100),
		mode1param: uint((instruction / 100) % 10),
		mode2param: uint((instruction / 1000) % 10),
		mode3param: uint((instruction / 10000) % 10),
	}
}

func RawCodeToInt(rawCode string) []int {
	codeSlitted := strings.Split(rawCode, ",")
	var code []int
	for _, codeString := range codeSlitted {
		resultInt, err := strconv.Atoi(codeString)
		if err == nil {
			code = append(code, resultInt)
		}
...
func IntCodeComputer(code []int, input int) {
	position := 0
	for position < len(code) {
		instruction := NewInstruction(code[position])
		if instruction.opcode == 1 || instruction.opcode == 2 || instruction.opcode == 5 || instruction.opcode == 6 || instruction.opcode == 7 || instruction.opcode == 8 {
			var a, b int
			if instruction.mode1param == 1 {
				a = code[position+1]
			} else {
				a = code[code[position+1]]
			}

			if instruction.mode2param == 1 {
				b = code[position+2]
			} else {
			...
			case 7:
				if a < b {
					code[code[position+3]] = 1
				} else {
					code[code[position+3]] = 0
				}
				position += 4
			case 8:
				if a == b {
			...
			code[code[position+1]] = input
			position += 2
		} else if instruction.opcode == 4 {
			if instruction.mode1param == 1 {
				fmt.Println(code[position+1])
			} else {
