...
func RawStringInstructions(instructionsRaw string) []Instruction {
	instructionsString := strings.Split(instructionsRaw, ",")
	var instructions []Instruction
	for _, s := range instructionsString {
		instruction, err := StringToInstruction(s)
		if err != nil {
			continue
		}
...
func StringToInstruction(instruction string) (Instruction, error) {
	if len(instruction) < 2 {
		return Instruction{}, errors.New("instruction string should have a direction and an amount (length of 2)")
	}
	direction := string(instruction[0])
	if string(instruction[1]) == "-" {
		return Instruction{}, errors.New("amount cant be negative")
	}
	...
	if direction == "D" || direction == "U" || direction == "L" || direction == "R" && amount >= 0 {
...
func InstructionsToCoordinates(instructions []Instruction) []Position {
	var positionsSlice []Position
	for _, instruction := range instructions {
		for i := 1; i <= int(instruction.Amount); i++ {
			switch instruction.Direction {
			case "U":
				if len(positionsSlice) > 0 {
					positionsSlice = append(positionsSlice, Position{
						X: positionsSlice[len(positionsSlice)-1].X,
						Y: positionsSlice[len(positionsSlice)-1].Y + 1,
					})
				} else {
					positionsSlice = append(positionsSlice, Position{
						X: 0,
			...
			case "L":
				if len(positionsSlice) > 0 {
					positionsSlice = append(positionsSlice, Position{
						X: positionsSlice[len(positionsSlice)-1].X - 1,
						Y: positionsSlice[len(positionsSlice)-1].Y,
					})
				} else {
					positionsSlice = append(positionsSlice, Position{
						X: -1,
...
func CrossingPointsFromPositionMaps(slice1, slice2 []Position) []Position {
	crossingPoint := make([]Position, 0, int(math.Max(float64(len(slice1)), float64(len(slice2)))))
	for _, m := range slice1 {
		for _, m2 := range slice2 {
			if m == m2 && m.X != 0 && m.Y != 0 {
				crossingPoint = append(crossingPoint, m)
			}
		}
	}
...
func MinManhattanDistance(reference Position, positions []Position) float64 {
	if len(positions) <= 0 {
		return 0
	}
	min := ManhattanDistance(reference, positions[0])
	for _, position := range positions {
		min = math.Min(min, ManhattanDistance(reference, position))
	}
	return min
}
...
func MinManhattanDistanceFromRawWiresPath(w1 string, w2 string) float64 {
	return MinManhattanDistance(Position{
		X: 0,
		Y: 0,
	}, CrossingPointsFromPositionMaps(InstructionsToCoordinates(RawStringInstructions(w1)), InstructionsToCoordinates(RawStringInstructions(w2))))
}

func StepsToCrossingPoint(point Position, paths [][]Position) int {
	steps := 2
	for _, path := range paths {
		for i, position := range path {
			if position == point {
				steps += i
			}
		}
...
func MinStepToCrossingPointFromRawWiresPath(w1 string, w2 string) int {
	w1Positions := InstructionsToCoordinates(RawStringInstructions(w1))
	w2Positions := InstructionsToCoordinates(RawStringInstructions(w2))
	crossingPoints := CrossingPointsFromPositionMaps(w1Positions, w2Positions)
	min := -1
	for _, point := range crossingPoints {
		if min < 0 {
			min = StepsToCrossingPoint(point, [][]Position{w1Positions, w2Positions})
		} else {
			min = int(math.Min(float64(min), float64(StepsToCrossingPoint(point, [][]Position{w1Positions, w2Positions}))))
		}
	}
	return min
}
